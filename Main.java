...
public class Main{
    ...
    public static ArrayList<String> filterWords(ArrayList<String> Words, ArrayList<String> knownLetters, ArrayList<String> Solution, ArrayList<String> invalidCharacters){
        ArrayList<String> reducedWords = new ArrayList<>();

        int SolutionKnownAmount = 5;
        for (int i = 0; i < Solution.size(); i++){
            if (Solution.get(i).equals("")) SolutionKnownAmount = SolutionKnownAmount - 1;
        }
        ...
        for (int i = 0; i < Words.size(); i++){
            ...
            for (int k = 0; k < 5; k++){
                for (int w = 0; w < knownLetters.size(); w++){
                    if (wordChar[k].equals(knownLetters.get(w))) match++;
                }
            }
            if (match < knownLetters.size()){
                ...
                if (matches == SolutionKnownAmount){
        ...
        for (String Word : Words){
            ...
            for (int i = 0; i < WordChar.length; i++){
                for (int k = 0; k < alphabetChar.length; k++){
                    if (WordChar[i].equals(alphabetChar[k])) Worth.set(k, Worth.get(k) + 1);
                }
            }
        }
        ...
        for (String Word : Words){
            ...
            for (int i = 0; i < Word.length(); i++){
                for (int k = 0; k < alphabetChar.size(); k++){
                    if (Word.charAt(i) == alphabetChar.get(k)) wordValue = wordValue + k;
                }
            }
    ...
    public static void main(String[] args) throws IOException{
        ...
        while(true){
            ...
            ArrayList<String> WordValues = generateWordValue(ReducedWords,generateCommonLetters(ReducedWords,generateLetterValue(ReducedWords)));
